...
from app.pc_control.windows_input import WindowsInputError, press_key, send_hotkey
...
KEY_COMMANDS: dict[str, tuple[list[str], list[str]]] = {
    "enter": (["enter"], ["нажми enter", "нажми энтер", "нажми интер", "нажми ентер", "нажми энтэр", "нажми клавишу enter", "нажми клавишу энтер", "нажми клавишу интер", "enter", "энтер", "интер", "ентер", "энтэр"]),
    "escape": (["escape"], ["нажми escape", "нажми esc", "нажми эскейп", "нажми ескейп", "нажми эск", "нажми клавишу escape", "нажми клавишу эскейп", "эскейп", "ескейп", "эск"]),
    "tab": (["tab"], ["нажми tab", "нажми таб", "нажми этап", "нажми тэб", "нажми клавишу tab", "нажми клавишу таб", "tab", "таб", "тэб"]),
    "backspace": (["backspace"], ["нажми backspace", "нажми бэкспейс", "нажми бекспейс", "нажми бэк спейс", "нажми бек спейс", "нажми клавишу backspace", "бэкспейс", "бекспейс"]),
    "delete": (["delete"], ["нажми delete", "нажми делит", "нажми дэлит", "нажми клавишу delete", "delete", "делит", "дэлит"]),
    "space": (["space"], ["нажми пробел", "нажми space", "нажми спейс", "пробел", "спейс"]),
    "home": (["home"], ["нажми home", "нажми хоум", "клавиша home", "клавиша хоум", "хоум"]),
    "end": (["end"], ["нажми end", "нажми энд", "клавиша end", "клавиша энд", "энд"]),
    "pageup": (["pageup"], ["нажми page up", "нажми pageup", "нажми пейдж ап", "страница вверх", "page up", "пейдж ап"]),
    "pagedown": (["pagedown"], ["нажми page down", "нажми pagedown", "нажми пейдж даун", "страница вниз", "page down", "пейдж даун"]),
    ...
    "select_all": (["ctrl", "a"], ["выдели все", "выдели всё", "контрол а", "контроль а", "ctrl a"]),
    "copy": (["ctrl", "c"], ["скопируй выделенное", "контрол с", "контроль с", "ctrl c"]),
    "paste": (["ctrl", "v"], ["вставь", "вставь сюда", "контрол в", "контроль в", "ctrl v"]),
    "cut": (["ctrl", "x"], ["вырежи", "контрол икс", "контроль икс", "ctrl x"]),
    "undo": (["ctrl", "z"], ["отмени последнее", "отмени действие", "контрол зет", "контроль зет", "ctrl z"]),
    "redo": (["ctrl", "y"], ["повтори действие", "верни отмененное", "верни отменённое", "ctrl y"]),
    "save": (["ctrl", "s"], ["сохрани документ", "сохрани файл", "контрол эс", "ctrl s"]),
    "find": (["ctrl", "f"], ["открой поиск", "найди на странице", "контрол эф", "ctrl f"]),
    ...
    "windows_search": (["win", "s"], ["поиск windows", "открой поиск windows", "открой поиск виндовс"]),
    ...
    "desktop_left": (["ctrl", "win", "left"], ["предыдущий рабочий стол", "рабочий стол влево", "переключи на предыдущий рабочий стол", "верни на предыдущий рабочий стол", "верни на старый рабочий стол", "вернись на старый рабочий стол"]),
    "desktop_right": (["ctrl", "win", "right"], ["следующий рабочий стол", "рабочий стол вправо", "переключи на следующий рабочий стол"]),
    "new_desktop": (["ctrl", "win", "d"], ["создай новый рабочий стол", "создай новый виртуальный рабочий стол", "новый рабочий стол", "новый виртуальный рабочий стол"]),
    "close_desktop": (["ctrl", "win", "f4"], ["закрой виртуальный рабочий стол", "закрой текущий рабочий стол", "закрой этот рабочий стол", "закрой рабочий стол"]),
    "fullscreen": (["f11"], ["полноэкранный режим", "на весь экран", "включи полный экран"]),
}

for function_number in range(1, MAX_FUNCTION_KEY + 1):
...
# F1..F12 remain individually addressable for the free local Snake route, but
# Melissa gets one compact validated action instead of twelve semantic actions.
for function_number in range(1, MAX_FUNCTION_KEY + 1):
    DEFAULT_TRIGGER_GROUPS[f"keyboard.f{function_number}"]["melissa_semantic"] = False

DEFAULT_TRIGGER_GROUPS["keyboard.function_key"] = {
    "display_name": "Нажать функциональную клавишу F1–F12",
    "triggers": [],
    "argument_hint": "arguments.target — номер функциональной клавиши от 1 до 12 или строка F1..F12.",
    "snake_triggers": False,
}
DEFAULT_TRIGGER_GROUPS["keyboard.desktop_number"] = {
    "display_name": "Переключиться на рабочий стол по номеру",
    "triggers": ["переключи на первый рабочий стол", "переключи на второй рабочий стол", "переключи на третий рабочий стол", "рабочий стол 1", "рабочий стол 2", "рабочий стол 3"],
...
class SystemKeyboardModule(AssistantModule):
    ...
    def execute_action(self, action_id: str, arguments: dict[str, Any] | None = None) -> ModuleResponse | None:
        args = arguments or {}
        ...
        if action_id == "keyboard.function_key":
            function_number = self._parse_function_key_target(args.get("target"))
            if function_number is None:
                return ModuleResponse(text="Назови функциональную клавишу от F1 до F12.")
            return self._execute_keys(f"f{function_number}", [f"f{function_number}"])
    ...
    def _execute_keys(self, action_id: str, keys: list[str]) -> ModuleResponse:
        ...
        responses = {
            "select_all": "Выделила всё.", "copy": "Скопировала выделенное.", "paste": "Вставила.", "cut": "Вырезала выделенное.",
            "undo": "Отменила последнее действие.", "redo": "Вернула отменённое действие.", "save": "Сохранила.", "find": "Открыла поиск.",
            "switch_window": "Переключаю окно.", "previous_window": "Переключаюсь на предыдущее окно.", "task_manager": "Открываю диспетчер задач.",
            "start_menu": "Открываю Пуск.", "windows_search": "Открываю поиск Windows.", "task_view": "Показываю открытые окна.",
            "snap_left": "Закрепляю активное окно слева.", "snap_right": "Закрепляю активное окно справа.",
            "desktop_left": "Переключаюсь на предыдущий рабочий стол.", "desktop_right": "Переключаюсь на следующий рабочий стол.",
            "new_desktop": "Создаю новый рабочий стол.", "close_desktop": "Закрываю текущий виртуальный рабочий стол.",
    ...
    def _find_action(self, text: str) -> tuple[str, list[str]] | None:
        ...
        for action_id, (keys, _default_triggers) in KEY_COMMANDS.items():
            for trigger in self.get_action_triggers(f"keyboard.{action_id}"):
    ...
    def _parse_desktop_target(self, value: object) -> int | None:
        ...
        return self._find_desktop_number(f"рабочий стол {text}") if text else None

    def _parse_function_key_target(self, value: object) -> int | None:
        if isinstance(value, bool):
            return None
        if isinstance(value, (int, float)) and float(value).is_integer():
            number = int(value)
            return number if 1 <= number <= MAX_FUNCTION_KEY else None
        text = self._normalize(str(value or ""))
        match = re.search(r"(?:^|\b)(?:f|эф)?\s*(1[0-2]|[1-9])(?:\b|$)", text)
        if not match:
            return None
