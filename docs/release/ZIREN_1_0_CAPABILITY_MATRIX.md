...
| Snake | custom trigger execution | 🟡 NEEDS FIX | route metadata is preserved; still verify every trigger-backed module reads TriggerStore at execution time |
...
3. Virtual desktop transitions and deterministic address-by-number behavior: 1→2→1→3→1.
...
6. Reminder/alarm delivery across Core restart and while TTS is busy; verify audible alarm and cancellation.
7. Power lock + one carefully controlled sleep/restart confirmation test.
8. Volume/mute against the real default endpoint.
9. DDC/CI brightness on supported and unsupported displays.
10. Physical two-monitor left/right move verification.
11. OneDrive/redirection test for Desktop/Documents/Pictures.
12. Staging Melissa smoke: natural-language commands for keyboard, windows, scheduler, brightness and clipboard all select only catalog actions.
